    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Log keypoints only if mode is 1 and user presses 'p' and a valid label is provided
                if mode == 1 and key == ord('p'):  # 'p' for logging keypoints
                    if custom_label is not None:
                        logging_csv(custom_label, mode, pre_processed_landmark_list, pre_processed_point_history_list)
                        print(f"Logged keypoints with label: {custom_label}")
                    else:
                        print("No valid label provided, unable to log keypoints.")

                # Hand sign classification
                hand_sign_id = keypoint_classifier(pre_processed_landmark_list)
                if hand_sign_id == "Not Valid":  # Point gesture
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                ...
                # Log keypoints if mode is 1 and user presses 'p'
                if mode == 1 and key == ord('p'):  # 'p' for logging keypoints
                    if custom_label is not None:
                        logging_csv(custom_label, mode, pre_processed_landmark_list, pre_processed_point_history_list)
                        print(f"Logged keypoints with label: {custom_label}")
